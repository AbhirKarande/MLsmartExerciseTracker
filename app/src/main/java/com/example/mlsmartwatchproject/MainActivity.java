...
public class MainActivity extends Activity implements SensorEventListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener{
    ...
    private Classifier cls_mul = null;
...
//        mApiClient.connect();
        ...
        try {
            cls_mul = (Classifier) read(getAssets().open("J48ExerciseNew4.model"));
...
//            }
            ...
            data = new String[2][3+1];
            features = new String[]{"mean_x", "median_z","mean_y","label"};
            data[0] = features;
            data[1][0] = String.valueOf(mean);  //mean_x
            data[1][1] = String.valueOf(m2);
            data[1][2] = String.valueOf(mean1);
            data[1][3] = "?";
            String arffData = null;
            try {
                arffData = MyWekaUtils.csvToArff(data, new int[]{0,1,2});
            ...
            Log.d(TAG, "Classification: " + unlabeled.classAttribute().value((int) clsLabel));
// for single model with multiple classes
            classification.setText("You are:" + clsLabel);
